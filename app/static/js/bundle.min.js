(() => {
  ...
  document.addEventListener("DOMContentLoaded", () => {
      ...
      if (!ticking) {
        window.requestAnimationFrame(() => {
          document.querySelectorAll("h1, h2").forEach((el) => {
            if (!el.classList.contains("in-view") && el.getBoundingClientRect().top < window.innerHeight - 60) {
              el.style.opacity = 1;
    ...
    if ("IntersectionObserver" in window) {
      ...
      document.querySelectorAll(".badge-glass, .prestige-badge").forEach((el) => badgeObserver.observe(el));
    }
    function animateFundraiserMeter() {
      ...
      setTimeout(() => {
        ...
        if (emojiLabel)
          emojiLabel.textContent = pct >= 100 ? "\u{1F3C6}" : pct >= 75 ? "\u{1F4AA}" : pct >= 50 ? "\u{1F525}" : pct >= 25 ? "\u{1F680}" : "\u{1F4A4}";
    ...
    if (toast && !sessionStorage.getItem("vipToastShown")) {
      toast.textContent = "\u{1F389} Welcome! New sponsors will be spotlighted here \u2014 you could be next!";
  ...
  window.openSpotlight = function(sponsorName = "A Generous Donor") {
    ...
    setTimeout(window.closeSpotlight, 4e3);
  };
  ...
  window.renderSponsorLeaderboard = function(sponsors = []) {
    ...
    if (sponsors.length === 0) {
      el.innerHTML = `<div class="col-span-2 text-center text-lg text-gold/80 font-semibold">Be our first sponsor! \u{1F3C5}</div>`;
      return;
    }
    el.innerHTML = sponsors.map(
      (s, i) => `
  ...
  window.renderSponsorLeaderboard([
    { name: "Gold\u2019s Gym", amount: 5e3 },
    { name: "Rodriguez Law", amount: 2500 },
    { name: "Redline BBQ", amount: 1e3 },
  ...
  window.sponsorAlert = function(name, tier = "Champion Sponsor") {
    ...
    <div class="bg-black border-4 border-gold rounded-2xl px-7 py-4 shadow-elevated flex flex-col items-center text-center">
      ...
      <span class="prestige-badge">${tier} \u{1F3C6}</span>
    </div>
  `;
