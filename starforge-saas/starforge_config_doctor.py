...
REQUIRED_CONFIG = {
    ...
    "SQLALCHEMY_DATABASE_URI": {"desc": "SQLAlchemy DB URI", "suggest": "mysql+pymysql://user:password@host/db"},
    "STRIPE_SECRET_KEY": {"desc": "Stripe Secret Key", "pattern": r"^sk_(test|live)_", "suggest": "sk_live_xxx"},
    "STRIPE_PUBLIC_KEY": {"desc": "Stripe Public Key", "pattern": r"^pk_(test|live)_", "suggest": "pk_live_xxx"},
    "STRIPE_WEBHOOK_SECRET": {"desc": "Stripe Webhook Secret", "pattern": r"^whsec_", "suggest": "whsec_xxx"},
    ...
    "MAIL_DEFAULT_SENDER": {"desc": "Default sender", "suggest": '"Team <team@email.com>"'},
...
OPTIONAL_CONFIG = {
    "REDIS_URL": {"desc": "Redis cache URL", "suggest": "redis://user:pass@host:6379/0"},
    "SENTRY_DSN": {"desc": "Sentry DSN", "suggest": "https://public@sentry.io/123"},
    "SLACK_WEBHOOK_URL": {"desc": "Slack Webhook", "suggest": "https://hooks.slack.com/services/XXX/YYY/ZZZ"},
    "OPENAI_API_KEY": {"desc": "OpenAI API Key", "suggest": "sk-xxxx"},
    "DOMAIN": {"desc": "Site domain for Stripe URLs", "suggest": "https://yourdomain.com"},
...
def check_config():
    ...
    for key, meta in REQUIRED_CONFIG.items():
        ...
        if not val:
            ...
            if 'pattern' in meta and not re.match(meta['pattern'], val):
                print(f"⚠️  FORMAT:  {key:25} — Value \"{val}\" does not match pattern {meta['pattern']}")
                errors += 1
            # URL check
    ...
    for key, meta in OPTIONAL_CONFIG.items():
        ...
        if val:
            if "URL" in key and not check_url(val):
                print(f"⚠️  OPTIONAL FORMAT: {key:25} — Value \"{val}\" is not a valid URL")
        else:
